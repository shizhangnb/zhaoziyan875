...
prmOm.sCodeName = 'a1.js';

/* SAME CODE BELOW THIS LINE */

prmOm.sCodeVersion = '0.1';
prmOm.sCodeBuild = '001';
...
if (prmOm.platformName == 'NextGen' || prmOm.platformName == 'corpSites') {
...
if (prmOm.pn == '') {
	...
	prmOm.saleButtonRegexp = /^(https?:\/\/((((((store\.kaspersky\.([^\/]+))|(gc\.digitalriver\.com)|(home\.kaspersky\.co\.jp))\/store\/kasper.+\/buy\/.+)|((store|secure|carrello)\.([a-z]{2,3}\.)?(kaspersky|avangate|2checkout)\.([^\/]+)\/order\/checkout\.php\?(.+\&))|(carrello\.kaspersky\.[a-z]{2}\/order\/trial\.php))\?PRODS(\[[^]]+\]|\d*)\=.+)|((boutique|eloja|store|tienda)(-ipp)?.kaspersky.([^\/]+)\/qte(_mvt)?.html\?(.+\&)?R\=.+)|(allsoft\.kaspersky\.[a-z]{2}\/basket\/add\/.+)|(softkey\.kaspersky\.[a-z]{2}\/\?(.+\&)?prodid\=.+)|((esd|secure)\.element5\.com\/(esales\/)?cart\.html\?(.+\&)?PRODUCT(\[|%5b)[^]]+(\]|%5d)\=.+)|(order\.shareit\.com\/cart\/add\?(.+\&)?PRODUCT\[[^]]+\]\=.+)|(kasperskystore\.pl\/addtocart\?(.+\&)?sku\=.+)))|(\/_svc\/price\.svc\/..-..\/route\.axd\?d\=.+)|(kaba365\.com\/mod\/(buy|cart)\.asp\?(parID|act)\=.+)|(\/mod\/(buy|cart)\.asp\?(parID|act)\=.+)|(kasperskyestore\.co\.kr\/mall\/products\/.+)|(shop\.kaspersky\.(.*))|(shop\.usa\.kaspersky\.com)/i
...
function omSetContext(omIframe) {
	...
	} else if (kaspersky.pageType == 'Product' && kaspersky.sku != undefined) { // product pages scenario
...
function omSetOmnitureParameters(tmpOm, instanceType, tmpContext) {
	...
	tmpOm.prop2 = String(kaspersky.pageType || 'other').toLowerCase();
	var businessValues = omGetBusinessType();
	tmpOm.prop3 = businessValues[0];
	tmpOm.prop20 = businessValues[1];
...
sng.visitor = Visitor.getInstance('983502BE532960BE0A490D4C@AdobeOrg', {
